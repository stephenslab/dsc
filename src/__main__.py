...
def remove(workflows, groups, modules, db, debug, replace = False, purge = False):
    ...
    if purge:
        cnt = 0
        for item in glob.glob(f"{db}/*"):
            if item not in [filename, filename[:-2] + 'map.mpk', filename[:-2] + 'conf.mpk']:
                os.remove(item)
                cnt += 1
...
def main():
    ...
    p.add_argument('--clean', metavar = "option", choices = ["remove", "replace", "purge"],
                   ...
                   "remove" deletes specified files, or files generated by specified modules in current DSC.
                   "replace" replaces these files by placeholder files with "*.zapped" extension,
                   ...
                   "purge" deletes ALL outputs in folder "DSC::output" regardless of specified targets and whether or not
                   they are generated by current DSC.''')
