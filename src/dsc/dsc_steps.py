...
class DSC2SoS:
    ...
    def __init__(self, data, echo = False):
        ...
        for seq in data.meta['sequences']:
            ...
            for idx in idxes:
                # A workflow
                workflow_id += 1
                wf_name = 'default.{}='.format(workflow_id)
                for x, y in zip(seq, idx):
                    step_name = '{}.{}'.format(x, y + 1)
                    wf_name += '{}+'.format(step_name)
                    if step_name in step_added:
                        continue
                    step_added.append(step_name)
                    steps.append(self.__get_script(step_name, data.data[x][y]))
    ...
    def __get_script(self, step_name, step_data):
