...
class DSCJobs(dotdict):
    ...
    def __update(self, block, dsc_block, name = 'block'):
        ...
        for idx, exe in enumerate(block.meta['exec']):
            ...
            if res['is_r']:
                res['r_end'].append('saveRDS(${_output_file!r}, %s)' % ', '.join(res['output_vars']))
                output_name = []
                for x in exe:
                    if x.startswith('$'):
                        break
                    output_name.append(re.sub('[^0-9a-zA-Z]+', '_', x.rsplit('/', 1)[-1]))
                res['output_base'].append('{}.rds'.format('_'.join(output_name)))
            # assign parameters
...
class DSC2SoS:
    ...
    def __init__(self, data, echo = False):
        ...
        keys = sorted(data.data.keys(),
                      key = lambda x: (data.meta["ordering"].index(x.rsplit('_', 1)[0]), int(x.rsplit('_', 1)[1])))
        steps = []
        for key in keys:
            steps.append(self.__get_step(key, data.data[key]))
        self.data = '{}\n{}'.\
    ...
    def __get_step(self, step_name, step_data):
        ...
        if len(step_data['output_base']):
            res.append('output_prefix = {}'.format(repr([x.rsplit('.', 1)[0] for x in step_data['output_base']])))
            res.append('output_suffix = {}'.format(repr([x.rsplit('.', 1)[1] for x in step_data['output_base']])))
        # FIXME: input and output
        params = sorted(step_data['parameters'].keys())
        res.append('input: for_each = {}'.format(repr(params)))
        res.append("""output_file = '${output_prefix}_%s.${output_suffix}'""" % \
                   ('_'.join(['${_%s}' % x for x in params])))
        # res.append("""output: '${output_prefix}_%s.${output_suffix}'""" % \
        #            ('_'.join(['${_%s}' % x for x in params])))
        if self.echo:
            # Debug and test
            res.append("""run('''\necho {}\necho Input:\n{}\necho Parameters:\n{}\necho Output:\n{}\n''')""".\
                       format(step_data['command'],
                              ...
                              '\n'.join(['echo "\toutput_file: ${output_file!r}"', 'touch ${output_file}'])
                              ))
        else:
            if step_data['is_r']:
                rscript = """R('''\n{3}\n{0}\n{4}\n{1}\n{3}\n{2}\n{4}\n''')""".\
